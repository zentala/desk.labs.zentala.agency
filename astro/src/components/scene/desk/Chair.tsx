...
function Cushions({ palette, chairStyle }: { palette: ScenePalette; chairStyle: ChairStyle }) {
    ...
    return (
      <>
        ...
        <group position={[0, SEAT_TOP - 0.03, SEAT[2] / 2 - 0.03]} rotation={[0.14, 0, 0]}>
          <Block size={[0.06, 0.16, 0.03]} position={[0, 0.06, 0]} color={palette.ink} />
  ...
  return (
    <>
      ...
      <Block size={[SEAT[0] - 0.14, 0.02, SEAT[2] - 0.14]} position={[0, SEAT_TOP - SEAT[1] - 0.008, 0]} color={palette.ink} castShadow={false} />
      <group position={[0, SEAT_TOP - 0.03, SEAT[2] / 2 - 0.03]} rotation={[0.14, 0, 0]}>
        <Block size={[0.06, 0.16, 0.03]} position={[0, 0.06, 0]} color={palette.ink} />
...
export function Chair({ t, palette, chairStyle = "sharp" }: ChairProps) {
  // pushed back and a little to the left as the person stands, so it stops being the biggest mass up front
  const z = CHAIR_Z + 0.45 * t;
  const yaw = 0.4 * t;
  const columnLen = SEAT_TOP - SEAT[1] - 0.06;
  return (
    <group position={[-0.15 * t, 0, z]} rotation={[0, yaw, 0]}>
